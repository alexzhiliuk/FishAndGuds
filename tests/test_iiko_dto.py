...
def test_organization_website_is_read_from_supported_iiko_shapes():
    direct = IikoOrganization.model_validate(
        {"id": "1", "name": "One", "websiteUrl": "https://one.example.com"}
    )
    nested = IikoOrganization.model_validate(
        {
            "id": "2",
            "name": "Two",
            "additionalInfo": {"website": "https://two.example.com"},
        }
    )
