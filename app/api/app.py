    ...
    *,
    ...
    if bot is not None and dispatcher is not None and webhook_secret:
        ...
        async def telegram_webhook(
            ...
            if cache is not None:
                claimed = await cache.claim(f"telegram:update:{update.update_id}", TELEGRAM_UPDATE_DEDUP_TTL_SECONDS)
                if not claimed:
            ...
            try:
                ...
                if "query is too old" not in str(exc).lower():
                    logger.exception("Telegram update failed update_id=%s", update.update_id)
                else:
                    logger.warning("Expired Telegram callback ignored update_id=%s", update.update_id)
            except Exception:
                ...
                logger.exception("Telegram update failed and will not be retried update_id=%s", update.update_id)
            return {"ok": True}
