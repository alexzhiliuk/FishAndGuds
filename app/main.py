...
async def wait_until_server_started(
    server: uvicorn.Server, task: asyncio.Task, timeout: float = 10.0
) -> None:
...
async def main():
    ...
    if not all(
        (
            settings.iiko_api_key,
            settings.iiko_app_id,
            settings.iiko_client_secret,
            settings.iiko_default_organization_id,
        )
    ):
        raise RuntimeError(
            "IIKO_API_KEY, IIKO_APP_ID, IIKO_CLIENT_SECRET and IIKO_DEFAULT_ORGANIZATION_ID are required in production"
        )
    ...
    dp = Dispatcher(
        storage=storage, events_isolation=RedisEventIsolation(redis=fsm_redis)
    )
    ...
    try:
        ...
        async with factory() as session:
            try:
                ...
                logging.exception(
                    "Initial iiko organization sync failed; scheduler will retry"
                )
        ...
        scheduler = configure_scheduler(
            bot=bot, session_factory=factory, iiko=iiko, settings=settings
        )
        scheduler.start()
        api = create_api(
            bot=bot,
        ...
        logging.info(
            "Application started in webhook mode: %s", settings.telegram_webhook_url
        )
        await api_task
    finally:
