...
class RealIikoClient(IikoClient):
    ...
    def _jwt_expiry(token: str) -> datetime:
        try:
            ...
            return datetime.fromtimestamp(
                json.loads(base64.urlsafe_b64decode(payload))["exp"], timezone.utc
            )
        except Exception:
    ...
    async def get_access_token(self) -> str:
        ...
        if (
            ...
            and self._token_expires_at > now + timedelta(minutes=2)
        ):
            return self._token
        try:
            ...
            raise IikoUnavailableError("iiko authentication is unavailable") from exc
        data = self._json(response)
        correlation_id = data.get("correlationId")
        logger.info(
            "iiko endpoint=/api/v2/access_token status=%s correlationId=%s",
            response.status_code,
            correlation_id,
        )
        if response.status_code >= 400 or not data.get("token"):
            raise IikoAuthenticationError(
                f"iiko authentication failed, status={response.status_code}, correlationId={correlation_id}"
            )
        self._token = data["token"]
        self._token_expires_at = self._jwt_expiry(self._token)
        return self._token

    async def _request(
        self, endpoint: str, payload: dict[str, Any], *, retry_unauthorized: bool = True
    ) -> dict[str, Any]:
        ...
        try:
            ...
            raise IikoUnavailableError(
                f"iiko endpoint unavailable: {endpoint}"
            ) from exc
        data = self._json(response)
        correlation_id = data.get("correlationId")
        logger.info(
            "iiko endpoint=%s status=%s correlationId=%s organizationId=%s",
            endpoint,
            response.status_code,
            correlation_id,
        ...
        if response.status_code in (401, 403):
            raise IikoAuthenticationError(
                f"iiko authorization failed, status={response.status_code}, correlationId={correlation_id}"
            )
        if response.status_code >= 500:
            raise IikoUnavailableError(
                f"iiko server error, status={response.status_code}, correlationId={correlation_id}"
            )
        if response.status_code >= 400:
            raise IikoRequestError(
                f"iiko request failed: {endpoint}",
    ...
    async def get_organizations(self):
        ...
        return [
            IikoOrganization.model_validate(
                {**item, "is_active": not item.get("isDisabled", False)}
            )
    ...
    async def get_customer_info(
        ...
        *,
        ...
        try:
            data = await self._request("/api/1/loyalty/iiko/customer/info", payload)
    ...
    async def create_or_update_customer(
        ...
        if customer.birthday is not None:
            payload["birthday"] = customer.birthday.strftime("%Y-%m-%d %H:%M:%S.%f")[
                :-3
            ]
        payload["organizationId"] = organization_id
        data = await self._request(
            "/api/1/loyalty/iiko/customer/create_or_update", payload
        )
    ...
    async def get_transactions_by_date(
        ...
        *,
        ...
        data = await self._request(
            ...
            {
                "customerId": customer_id,
                "dateFrom": date_from.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
                "dateTo": date_to.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3],
        ...
        return [
            LoyaltyTransaction.model_validate(item)
    ...
    async def get_transactions_by_revision(
        ...
        *,
        ...
        data = await self._request(
            "/api/1/loyalty/iiko/customer/transactions/by_revision", payload
        )
        return (
            [
                LoyaltyTransaction.model_validate(item)
            ...
            int(data.get("lastRevision", revision)),
