...
def create_iiko_client(
    settings: Settings, cache: RedisJsonCache | None = None
) -> IikoClient:
    backend = RealIikoClient(
    ...
    if cache is None:
        return backend
    return CachedIikoClient(
        backend, cache, organizations_ttl=settings.iiko_organizations_cache_ttl_seconds
    )
