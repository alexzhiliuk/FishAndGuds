...
class CachedIikoClient(IikoClient):
    ...
    def __init__(self, backend: IikoClient, cache: RedisJsonCache, *, organizations_ttl: int):
    ...
    async def get_transactions_by_date(self, *, customer_id: str, date_from: datetime, date_to: datetime, page_number: int, page_size: int, organization_id: str):
        return await self.backend.get_transactions_by_date(customer_id=customer_id, date_from=date_from, date_to=date_to, page_number=page_number, page_size=page_size, organization_id=organization_id)
    ...
    async def get_transactions_by_revision(self, *, customer_id: str, revision: int, last_transaction_id: str | None, page_size: int, organization_id: str):
        return await self.backend.get_transactions_by_revision(customer_id=customer_id, revision=revision, last_transaction_id=last_transaction_id, page_size=page_size, organization_id=organization_id)
