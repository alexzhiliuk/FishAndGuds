...
def loyalty_terms_keyboard(rules_url: str | None):
    rows = []
    if rules_url:
        rows.append([InlineKeyboardButton(text="📜 Ознакомиться с условиями", url=rules_url)])
...
def action_keyboard(label: str, url: str | None, action: str):
    ...
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data=f"restaurants:{action}")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def mailing_actions(item_id: int, status: str, list_page: int = 0):
    ...
    rows.append([InlineKeyboardButton(text="⬅️ К списку", callback_data=f"admin:list_back:{list_page}")])
