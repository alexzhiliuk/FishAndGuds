...
class LoyaltyService:
    def __init__(self, session: AsyncSession, iiko: IikoClient | None = None, default_organization_id: str | None = None):
    ...
    async def get_profile(self, telegram_id: int):
        ...
        if not user or not user.loyalty_account: return None
        account = user.loyalty_account
        if self.iiko and self.default_organization_id and account.iiko_customer_id:
            try:
                customer = await self.iiko.get_customer_info(organization_id=self.default_organization_id, customer_id=account.iiko_customer_id)
                if customer:
                    await CustomerService(self.session, self.iiko, self.default_organization_id).sync_customer(user, customer)
                    await CardService(self.session, self.iiko, self.default_organization_id).sync_card(user, customer)
    ...
    def generate_qr(payload: str) -> bytes:
        image = qrcode.make(payload); stream = io.BytesIO(); image.save(stream, format="PNG"); return stream.getvalue()
...
class RestaurantService:
    def __init__(self, session: AsyncSession): self.session = session
    async def list_active(self): return await RestaurantRepository(self.session).active()
    async def list_all(self): return await RestaurantRepository(self.session).list_all()
    async def get(self, item_id: int): return await RestaurantRepository(self.session).by_id(item_id)
    async def update_local_link(self, item_id: int, field: str, value: str | None):
        item = await RestaurantRepository(self.session).update_local_link(item_id, field, value)
...
class NotificationService:
    ...
    async def toggle(self, telegram_id: int, key: str):
        user = await UserRepository(self.session).by_telegram_id(telegram_id)
        field = self.FIELDS[key]; setattr(user.notification_settings, field, not getattr(user.notification_settings, field)); await self.session.commit(); return user.notification_settings
...
class SyncService:
    ...
    async def sync_all(self):
        ...
        for user in await UserRepository(self.session).active():
            if user.loyalty_account: total += await self.sync_user(user)
        logger.info("Sync completed, imported=%s", total); return total


class MailingService:
    def __init__(self, session: AsyncSession): self.session = session
    async def create(self, name: str, text: str, image_file_id: str | None = None):
        item = await MailingRepository(self.session).create(name=name, text=text, image_file_id=image_file_id, type=MailingType.manual, status=MailingStatus.draft); await self.session.commit(); return item
    async def update(self, item_id: int, **changes):
        ...
        if item.status == MailingStatus.sending: raise ValueError("Нельзя редактировать рассылку во время отправки")
        reset_to_draft = item.status in (MailingStatus.sent, MailingStatus.cancelled, MailingStatus.failed) and "status" not in changes
        for key, value in changes.items(): setattr(item, key, value)
        if reset_to_draft:
            item.status = MailingStatus.draft
            item.scheduled_at = None
        await self.session.commit(); return item
    async def schedule(self, item_id: int, when: datetime): return await self.update(item_id, scheduled_at=when, type=MailingType.scheduled, status=MailingStatus.scheduled)
    async def cancel(self, item_id: int): return await self.update(item_id, status=MailingStatus.cancelled)
    async def delete(self, item_id: int):
        repo = MailingRepository(self.session); item = await repo.get(item_id)
        if not item: raise ValueError("Рассылка не найдена")
        if item.status == MailingStatus.sending: raise ValueError("Нельзя удалить рассылку во время отправки")
        await repo.delete(item); await self.session.commit()
    async def list(self): return await MailingRepository(self.session).list()
    async def page(self, page: int, size: int = 6): return await MailingRepository(self.session).page(page, size)
    async def get(self, item_id: int): return await MailingRepository(self.session).get(item_id)
    async def send(
        self,
        item_id: int,
        ...
        try:
            for user in users:
                try: await sender(user.telegram_id, item.text, item.image_file_id); run.sent_count += 1
                except Exception: run.failed_count += 1; logger.exception("Mailing delivery failed user_id=%s", user.id)
            run.status = RunStatus.sent; item.status = MailingStatus.sent
        except Exception as exc:
            run.status = RunStatus.failed; item.status = MailingStatus.failed; run.error = str(exc)[:2000]; logger.exception("Mailing failed id=%s", item.id)
        run.finished_at = datetime.now(timezone.utc); await self.session.commit(); return run
