...
class RegistrationService:
    ...
    def _customer_service(self):
        return CustomerService(self.session, self.iiko, self.default_organization_id)
    ...
    async def start(self, telegram_id: int, phone: str):
        phone = PhoneNormalizationService.normalize(phone)
        repo = UserRepository(self.session)
        if user := await repo.by_telegram_id(telegram_id):
            return RegistrationStart(user=user)
        if await repo.by_phone(phone):
            raise ValueError("Этот номер уже связан с другим Telegram-пользователем")
        try:
            customer = await self._customer_service().get_by_phone(phone)
        except IikoError:
            return RegistrationStart(needs_form=True, iiko_available=False)
        if customer is None:
            return RegistrationStart(needs_form=True)
        birthday = (
    ...
    async def complete(
        ...
        *,
        ...
        phone = PhoneNormalizationService.normalize(phone)
        repo = UserRepository(self.session)
        if existing := await repo.by_telegram_id(telegram_id):
            return existing
        if await repo.by_phone(phone):
            raise ValueError("Этот номер уже связан с другим Telegram-пользователем")
        user = await repo.create_local(
            ...
            sync_status=IikoSyncStatus.pending,
    ...
    async def sync_pending_user(self, user):
        try:
            ...
            logger.info(
                "Pending iiko registration synced user_id=%s iiko_customer_id=%s organizationId=%s",
                user.id,
            ...
            logger.warning(
                "Pending iiko registration deferred user_id=%s organizationId=%s error=%s",
                user.id,
                self.default_organization_id,
                type(exc).__name__,
            )
            ...
            logger.exception(
                "Pending iiko registration failed user_id=%s organizationId=%s",
                user.id,
                self.default_organization_id,
            )
            await self.session.commit()
